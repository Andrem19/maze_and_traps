...
class MapTrainingActController extends GetxController {
  ...
  FirebaseFirestore firebaseFirestore = FirebaseFirestore.instance;
  MainGameController mainCtrl = Get.find<MainGameController>();
  Rx<GameInfo> gameInfo =
      GameInfo.createEmptyGameInfo(Get.find<MainGameController>().currentGameMap!).obs;
  String mapId = '';
  int durationOfAct = 600;
  late int time;
  Duration clockTimer = Duration(seconds: 600);
  ...
  Future<void> onInit() async {
    ...
    if (mainCtrl.currentGameMap != null) {
      ...
      mazeMap.value = EditorPageMap.createStruct(TestData.createTestMap());
    }
    ...
    _timer = Timer.periodic(Duration(milliseconds: timerDuration), (timer) async {
      await timerCode();
    });
    super.onInit();
  }
  ...
  bool isPlayerWinn() => mazeMap.value.Player_A_Coord == mazeMap.value.Player_B_Coord;
